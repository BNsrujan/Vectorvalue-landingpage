...
export function MegaMenu({ groups = [], note, ctaLabel = "Book a Call", ctaHref = "/book-a-call", onNavigate, style, ...rest }) {
      ...
      <div style={{ maxWidth: "var(--container-max)", margin: "0 auto", padding: "var(--space-10) var(--container-gutter)", display: "grid", gridTemplateColumns: "var(--mega-cols)", gap: "var(--space-10)" }}>
        {groups.map((g) => (
          <div key={g.title} style={{ display: "flex", flexDirection: "column", gap: "var(--space-4)" }}>
            <a href={g.href} onClick={(e) => go(e, g.href)} style={{ display: "flex", alignItems: "center", gap: 10, textDecoration: "none", color: "var(--text-primary)" }}>
            ...
            <ul style={{ listStyle: "none", margin: 0, padding: 0, display: "flex", flexDirection: "column", gap: 2 }}>
              {g.items.map((it) => (
                <li key={it.label}>
                  <a href={it.href || g.href} onClick={(e) => go(e, it.href || g.href)}
        ...
        <div style={{ background: "var(--ink-800)", padding: "var(--space-8)", display: "flex", flexDirection: "column", justifyContent: "space-between", gap: "var(--space-6)", backgroundImage: "var(--pattern-blueprint-dark)", backgroundSize: "var(--grid-size) var(--grid-size)" }}>
          ...
          <a href={ctaHref} onClick={(e) => go(e, ctaHref)} style={{ display: "inline-flex", alignItems: "center", gap: 10, alignSelf: "flex-start", padding: "12px 20px", background: "var(--orange-600)", color: "var(--text-on-accent)", textDecoration: "none", fontFamily: "var(--font-core)", fontSize: "var(--text-caption)", fontWeight: 600, borderRadius: "var(--radius-1)" }}>
